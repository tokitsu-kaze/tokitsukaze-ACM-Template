...
const int DIGIT=9;// 注意对应 DEPTH
const int MAXN=200;//题目最大位数/DIGIT 注意memset的时间 
...
/* 组合数															   */
/************************************************************************/
void combination(bignum_t a,ll m,ll n)
{
	ll*t=new ll[m+1];
	memset((void*)t,0,sizeof(ll)*(m+1));
	comp(t,n+1,m,1);
	comp(t,2,m-n,-1);
	convert(t,m,a);
	delete[]t ;
...
/* 排列数															   */
/************************************************************************/
void permutation(bignum_t a,ll m,ll n)
{
	ll i,t=1 ;
...
struct bigint
{
	...
	inline bigint C(const ll m,const ll n)
	{
		combination(num,m,n);
		sgn=1 ;
		return*this ;
	}
	inline bigint P(const ll m,const ll n)
	{
		permutation(num,m,n);
		sgn=1 ;
		return*this ;
